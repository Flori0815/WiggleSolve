...
### 2D mode as a constraint layer:
When a system is flagged as "dimensions": "2D" in the JSON definition, the solver engine and the UI enforce strict constraints:
...
Promoting a 2D definition to 3D is trivial. The JSON is imported, and the constraints locking the Z, Rx, and Ry axes are simply removed or exposed to the user to adjust.

### 3D to 2D Projection:
Projecting a 3D system into 2D involves an orthographic projection. The system flattens the Z-coordinates of all Nodes to a defined plane (e.g., $Z=0$) and maps the 3D rotational frames to pure Z-axis rotations relative to the viewer's normal plane.
...
Because the solver runs entirely client-side using core-js, the tooling-ui React application will be compiled into static files and hosted directly and freely via GitHub Pages.
